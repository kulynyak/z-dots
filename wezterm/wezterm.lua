...
local function format_title(tab)
  ...
  local description = (not active_title or active_title == cwd) and "~" or
                        active_title
...
local function string_to_color(str)
  ...
  local hash = 0
  for i = 1, #str do hash = string.byte(str, i) + ((hash << 5) - hash) end
...
function table_contains(tbl, element)
  for _, value in pairs(tbl) do if value == element then return true end end
  return false
end

-- Function to toggle the light color scheme
function toggle_light_scheme(window) set_color_scheme(window, light_scheme) end

-- Function to toggle the dark color scheme
function toggle_dark_scheme(window) set_color_scheme(window, dark_scheme) end
